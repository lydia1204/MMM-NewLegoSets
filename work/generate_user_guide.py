...
def build_story():
    ...
    cover_copy = Table([
        ...
        [p("10 layouts  |  11 themes  |  22 animations  |  1-10 visible products", "CoverSub")],
    ], colWidths=[3.5 * inch])
    ...
    story += [Spacer(1, 8), p("The two clocks", "H2x"), table([["Setting", "Default", "Meaning"], ["data.pollInterval", "6 hours", "Download a fresh list from LEGO.com."], ["cycle.interval", "12 seconds", "Change the visible set/group without downloading."]], [1.8 * inch, 1.1 * inch, 3.5 * inch]), p("Use milliseconds. For readability, write <b>4 * 60 * 60 * 1000</b> for four hours and <b>15 * 1000</b> for fifteen seconds.", "Callout"), PageBreak()]
    ...
    story += [p("4. Motion and LEGO build", "H1x"), p("Twenty-two choices cover restrained fades through expressive brick construction. The module honors the operating system's reduced-motion preference by default.")]
    animation_groups = [
        ...
        ("REVEALS", "wipe, shutter", colors.HexColor("#087F5B")), ("BRICKS", "legoBuild, legoBreakBuild, random", LEGO_RED),
    ]
    story += [colored_cards(animation_groups), Spacer(1, 8), code('animation: {\n  name: "legoBreakBuild",\n  duration: 1100,\n  easing: "cubic-bezier(0.22, 1, 0.36, 1)",\n  stagger: 18,\n  particleCount: 36,\n  brickSize: 10,\n  respectReducedMotion: true\n}')]
    story += [p("Performance recipe", "H2x"), p("For Raspberry Pi-class mirrors, start with fade, 600-800 ms, and 12-24 particles. High particle counts, blur, and large shadows increase GPU work. Keep reduced-motion support on unless you know the viewer's needs."), PageBreak()]
    ...
    story += [Spacer(1, 9), table([["Cycle key", "Default", "Explanation"], ["enabled", "true", "Automatic slide movement."], ["interval", "12 seconds", "Time between visible changes."], ["step", "1", "Positions advanced each cycle."], ["loop", "true", "Wrap at the end."], ["shuffle", "false", "Shuffle each fresh pool."], ["pauseOnHover", "false", "Pause for pointer/touchscreen inspection."], ["indicatorStyle", "dots", "Dots, bars, numbers, or none."]], [1.75 * inch, 1.1 * inch, 3.55 * inch])]
    ...
    story += [table([["Symptom", "Check"], ["Only one set", "Raise productCount to show more; or keep 1 and ensure poolSize > 1 for cycling."], ["Sets never change", "cycle.enabled, cycle.interval, and poolSize larger than productCount."], ["Images blank", "Access to www.lego.com/cdn and DNS filtering."], ["Dates blank", "Expected without trustworthy data; use Brickset or overrides."], ["Motion missing", "The operating system may request reduced motion."], ["Too many dots", "Raise cycle.step, switch to bars, or hide indicators."], ["Narrow overflow", "Use auto/compact/table, fewer columns, or smaller cardMinWidth."], ["Custom font missing", "Use a reachable WOFF2 path, preferably local."], ["Brickset warning", "Check key/connectivity; products still render without enrichment."], ["Listing parser error", "Run validate:live and inspect MagicMirror's terminal."]], [1.75 * inch, 4.65 * inch])]
    story += [Spacer(1, 10), p("Validated release matrix", "H2x"), p("10 desktop layouts + 10 narrow layouts; all 11 themes; product counts 1-10; all 22 animation states; 36-particle LEGO build; real single-product advancement; current and legacy LEGO payloads; live 10-product fetch; syntax and boundary validation.", "Callout"), PageBreak()]
    ...
    story += [Spacer(1, 12), colored_cards([
        ...
        ("WATCH THE CLOCKS", "pollInterval fetches; cycle.interval slides", PURPLE),
