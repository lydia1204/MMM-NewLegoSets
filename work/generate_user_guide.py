...
def page_header_footer(canvas, doc):
    ...
    canvas.setFillColor(INK)
    canvas.drawString(0.55 * inch, 0.33 * inch, "MMM-NewLegoSets 2.1 User Guide")
...
def product_image():
    ...
    try:
        request = urllib.request.Request(url, headers={"User-Agent": "MMM-NewLegoSets-Guide/2.1"})
...
def build_story():
    ...
    cover_copy = Table([
        ...
        [p("THE COMPLETE 2.1 USER GUIDE", "CoverSub")],
    ...
    story.append(colored_cards([
        ...
        ("RECENT BY DESIGN", "A 31-day window prioritizes exact official announcements and releases.", LEGO_RED),
    ...
    story += [Spacer(1, 0.25 * inch), p("Version 2.1 | August 2026", "H2x"), p("This manual accompanies the exhaustive README in the module folder. It focuses on setup, recent-product rules, visual choices, common recipes, and quick reference.", "Bodyx"), PageBreak()]
    ...
    story += [colored_cards(layout_items), Spacer(1, 8), p("Sizing controls", "H2x"), table([["Setting", "Use"], ["productCount", "Visible products, 1 through 10."], ["layoutSettings.columns", "Maximum grid and masonry columns, 1 through 10."], ["moduleWidth / moduleMaxWidth", "Region width and maximum pixel width."], ["cardMinWidth", "Preferred card width; caps columns without forcing overflow."], ["gap / padding / cardPadding", "Density and breathing room."], ["imagePosition / alignItems", "Card composition and vertical alignment."]], [2.3 * inch, 4.1 * inch]), PageBreak()]
    ...
    story += [p("The full brick wall", "H2x"), p("brickWallRebuild clips the complete rendered module - header, image, text, indicators, footer, and decorations - into a wall. The outgoing wall falls below the module; the incoming wall descends from above and resolves into one solid module."), p("Leave <b>animation.brickColors</b> empty to use the active theme palette. Supply 1-16 hex colors to override it. The trans theme defaults to #5BCEFA, #F5A9B8, and #FFFFFF.", "Callout")]
    ...
    story += [table([["Data key", "Default", "Explanation"], ["locale / countryCode", "en-us / US", "Storefront, formatting, and Brickset region."], ["pageCount", "2", "Listing pages attempted, 1-8."], ["poolSize", "10", "Up to 50; page mode reserves two full pages."], ["coming soon / preorders", "true / true", "Include announced and preorder products."], ["recentOnly / recentDays", "true / 31", "Enable and size the recent window."], ["unknownDatePolicy", "firstSeen", "Fallback, include, or strict exclude."], ["newsroomAnnouncements", "true", "Match official article dates by set number."], ["pollInterval", "6 hours", "Successful refresh clock."], ["cacheEnabled / max age", "true / 7 days", "Persist and reuse the last good pool."], ["sortBy / direction", "recent / desc", "Exact announcements first and newest first."]], [1.75 * inch, 1.1 * inch, 3.55 * inch], font_size=6.8)]
    story += [Spacer(1, 9), table([["Cycle key", "Default", "Explanation"], ["enabled", "true", "Automatic slide movement."], ["mode", "transition", "Timed effects or continuous scroll."], ["interval", "12 seconds", "Delay between transition-mode changes."], ["scrollSpeed", "60 px/s", "Continuous speed, 10-500."], ["scrollDirection", "left", "Left, right, up, or down."], ["step", "page", "Replace the complete visible group."], ["loop", "true", "Wrap at the end."], ["shuffle", "false", "Shuffle each fresh pool."], ["indicatorStyle", "dots", "Dots, rings, shapes, bars, numbers, or none."]], [1.75 * inch, 1.1 * inch, 3.55 * inch], font_size=6.9)]
    ...
    story += [Spacer(1, 9), p("Page-sized slideshow", "H2x"), code('productCount: 2,\ndata: { poolSize: 10, pollInterval: 6 * 60 * 60 * 1000 },\ncycle: { enabled: true, interval: 12 * 1000, step: "page" }'), p("With one product, page advances by one. With two products, both are replaced. Numeric steps 1-10 remain available for intentional overlapping windows.", "Callout"), PageBreak()]
    ...
    story += [colored_cards([
        ...
        ("NEWSROOM", "Official article dates are matched to exact set numbers.", colors.HexColor("#087F5B")),
        ("BRICKSET", "Optional regional dateFirstAvailable or launchDate enrichment.", PURPLE),
        ("FIRST SEEN", "A transparent fallback for new-listing products with no exact date.", LEGO_RED),
    ])]
    story += [Spacer(1, 8), code('data: {\n  recentOnly: true,\n  recentDays: 31,\n  unknownDatePolicy: "firstSeen",\n  newsroomAnnouncements: true,\n  newsroomPageLimit: 30,\n  sortBy: "recent",\n  sortDirection: "desc"\n}')]
    story += [p("Exact announcements sort first, then exact past releases, then first-seen fallbacks. Shipping and backorder estimates are ignored. First-seen time is never displayed as an announcement or release date. Choose unknownDatePolicy: \"exclude\" when only exact dates are acceptable.", "Callout"), PageBreak()]

    story += [p("8. Ready-made recipes", "H1x")]
    story += [p("Ten-product Progress Pride grid", "H2x"), code('layout: "grid",\nproductCount: 10,\ntheme: "progress",\ndata: { poolSize: 20 },\ncycle: { interval: 30000, step: "page", indicatorStyle: "bars" },\nlayoutSettings: { columns: 2, moduleMaxWidth: 900 }')]
    ...
    story += [table([["Symptom", "Check"], ["Only one set", "Raise productCount to show more; or keep 1 and ensure poolSize > 1 for cycling."], ["Sets never change", "cycle.enabled, mode, interval/speed, and poolSize larger than productCount."], ["Images blank", "Access to www.lego.com/cdn and DNS filtering."], ["Dates blank", "No exact source matched; choose fallback/strict policy, Brickset, or overrides."], ["Motion missing", "The operating system may request reduced motion."], ["Too many symbols", "Keep step page, switch to dots/bars, or hide indicators."], ["Scroll will not start", "Use hero, carousel, or filmstrip and keep cycling enabled."], ["Narrow overflow", "Use auto/compact/table, fewer columns, or larger cardMinWidth."], ["Custom font missing", "Use a reachable WOFF2 path, preferably local."], ["Brickset warning", "Check key/connectivity; products still render without enrichment."], ["Listing parser error", "Run validate:live and inspect MagicMirror's terminal."]], [1.75 * inch, 4.65 * inch], font_size=7.0)]
    story += [Spacer(1, 10), p("Validated release matrix", "H2x"), p("5,584 pairwise normalized states and 1,249 rendered editor states; all 10 layouts, 11 themes, 23 animations, 11 indicators, product counts 1-10, fields, text effects, device presets, and boundaries. All 69 two-product hero/carousel/filmstrip transition lifecycles and all 12 continuous-scroll layout/direction cases passed. A live 31-day feed check returned 47 unique canonical set numbers and exact Newsroom announcements newest-first on August 16, 2026.", "Callout"), PageBreak()]
    ...
    story += [Spacer(1, 18), p("Documentation and community references", "H2x"), p("MagicMirror core module, rendering, and animation documentation; Brickset API v3 documentation; official LEGO Newsroom; MMM-Carousel; and MagicMirror community discussion of CSS animation performance. Full links are in README.md.")]
...
def main():
    ...
    document = SimpleDocTemplate(
        ...
        title="MMM-NewLegoSets 2.1 User Guide",
